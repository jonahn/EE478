...
#if defined(DPSLP_V1_1) || defined (DPSLP_V2_1)
...
{ 
    ...
    {
        unsigned char    DS_POR:1;      //wake up source is Power on reset 
		unsigned char    DS_ICD:1;		//wake up source is In-circuit debugger
        unsigned char    DS_MCLR:1;		//wake up source is master reset pin set to low
        unsigned char    DS_RTC:1;		//wake up source is rtcc alarm
        unsigned char    DS_WDT:1;		//wake up source is deep sleep watch dog timer expiry
		unsigned char    DS_EXT:1;		//wake up source is external interrupt
		unsigned char    DS_BOR:1;		//Brown out occured during deep sleepunsigned char    DS_FLT:1;		//wake up due to fault in deep sleep configuration corruption
		unsigned char    DS_FLT:1;		//wake up due to fault in deep sleep configuration corruption
		unsigned char    DS_INT0:1;		//wake up source is external interrupt
...
{
		unsigned char	Reg0;			//Stores the DSGPR0 register contents
		unsigned char	Reg1;			//Stores the DSGPR1 register contents
		unsigned char	Reg2;			//Stores the DSGPR2 register contents
		unsigned char	Reg3;			//Stores the DSGPR3 register contents
}CONTEXT;
...
/**********************************************************************
 Macro       : Write_DSGPR(data1,data2,data3,data4)
 ...
 Remarks     : None
**********************************************************************/                        
 #define Write_DSGPR(data1,data2,data3,data4)          DSGPR0=data1;DSGPR1=data2;DSGPR2=data3;DSGPR3=data4
